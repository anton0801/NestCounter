...
final class HTTPNetworkService: NetworkService {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(NestCounterConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: NestCounterConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://nesttcounter.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(NestCounterConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [33.0, 66.0, 132.0]
